...
func daemon() {
	...
	var (
		...
		checkKeysBatchSize            = flag.Int64("check-keys-batch-size",
			getEnvInt64("REDIS_EXPORTER_CHECK_KEYS_BATCH_SIZE", 1000),
		...
		tlsServerCaCertFile = flag.String("tls-server-ca-cert-file",
			getEnv("REDIS_EXPORTER_TLS_SERVER_CA_CERT_FILE", ""),
		...
		maxDistinctKeyGroups = flag.Int64("max-distinct-key-groups",
			getEnvInt64("REDIS_EXPORTER_MAX_DISTINCT_KEY_GROUPS", 100),
			"The maximum number of distinct key groups with the most memory utilization to present as distinct metrics per database, the leftover key groups will be aggregated in the 'overflow' bucket")
		...
		exportClientPort = flag.Bool("export-client-port",
			getEnvBool("REDIS_EXPORTER_EXPORT_CLIENT_PORT", false),
